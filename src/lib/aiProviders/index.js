 ...
 *   analyzeStudent({ apiKey, baseUrl?, model, temperature?, maxTokens?, presencePenalty?, frequencyPenalty?, text, systemPrompt? }): Promise<string>  — plain-text analysis of one student's anonymized engagement situation
 *
 ...
 *
 * `systemPrompt` on all four defaults to that capability's own constant
 * (SYSTEM_PROMPT / REPLY_SYSTEM_PROMPT / IMPROVE_SYSTEM_PROMPT /
 * STUDENT_ANALYSIS_SYSTEM_PROMPT) when omitted
 ...
 */
export const PROVIDERS = [openai, gemini, claude, zai, deepseek];
